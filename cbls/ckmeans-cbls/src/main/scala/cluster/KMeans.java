...
public class KMeans {
	...
	/**
	 * Empty constructor is private to ensure that clients have to use the Builder
	 * inner class to create a KMeans object.
	 */
	private KMeans() {
	}
	...
	private void updateStep() {
		...
		// gracefully handle empty clusters by assigning to that centroid an unused data
		// point
	 ...
	 **********************************************************************/
	/**
	 * Uses either plusplus (KMeans++) or a basic randoms sample to choose initial
	 * centroids
	 */
	...
	/**
	 * Randomly chooses (without replacement) k data points as initial centroids
	 * using a weighted probability distribution (proportional to D(x)^2 where D(x)
	 * is the distance from a data point to the nearest, already chosen centroid).
	 */
	 ...
	 * 
	 * @param prevWCSS
	 *            error from previous step in the run
	 * @return
	 */
	private boolean stop(double prevWCSS) {
		...
		// I think not because WCSS is calc in same way as prevWCSS (if data structs
		// don't change)
	}

	/**
	 * Signals to stop running KMeans when the marginal improvement in WCSS from the
	 * last step is small.
	 * 
	 * @param prevWCSS
	 *            error from previous step in the run
	...
	private static class Distance {
		 ...
		 * <P>
		 * Euclidean distance, or Minkowski distance of order 2.
		 * 
		 * @param x
		 * @param y
		 * @return
	...
	/**
	 * Calculates WCSS (Within-Cluster-Sum-of-Squares), a measure of the
	 * clustering's error.
	 */
